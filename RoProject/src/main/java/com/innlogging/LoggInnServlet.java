...
public class LoggInnServlet extends HttpServlet {
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if(brukernavn.contains("utover")){
            Cookie coockie = new Cookie("UID", passord + ""); //Lagrer passordet(UID) i en cookie
            ...
            Cookie cookie = new Cookie("KLUBBID", passord + ""); //Lagrer passordet(KlubbID) i en cookie
            response.addCookie(cookie);
            response.sendRedirect("hentTrener"); //Sender til innloggingServlet som "hentEn key"
        }
        else if(brukernavn.contains("super")){
            response.sendRedirect("superPage.jsp"); //Sender til superPage så lenge brukernavnet er "super"
        }
    }
